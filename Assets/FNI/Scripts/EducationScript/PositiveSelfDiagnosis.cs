...
namespace FNI
{
    public class PositiveSelfDiagnosis : BaseScript
    {
        #region Singleton
        private static PositiveSelfDiagnosis _instance;
        public static PositiveSelfDiagnosis Instance
        {
            get
            {
                if (_instance == null)
                {
                    _instance = FindObjectOfType<PositiveSelfDiagnosis>();
                    if (_instance == null)
                        Debug.LogError("PositiveSelfDiagnosis를 찾을 수 없습니다. ");
                }
                return _instance;
            }
        }
