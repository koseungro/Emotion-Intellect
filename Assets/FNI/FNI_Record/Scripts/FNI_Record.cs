...
public class FNI_Record : MonoBehaviour
{
    ...
    void Start ()
    {
        ...
        m_start = m_canvas.transform.Find("Buttons/Start").GetComponent<Button>();
        m_stop = m_canvas.transform.Find("Buttons/Stop").GetComponent<Button>();
        m_replay = m_canvas.transform.Find("Buttons/Replay").GetComponent<Button>();

        m_recordBG = m_canvas.transform.Find("RecordBG").gameObject;
    ...
    public void BackGroundShow(bool value = false)
    {
        m_recordBG.SetActive(value);
    }
    ...
    /// 녹음을 시작합니다.
    /// </summary>
    private void Start_Record()//녹음 시작
    {
        //초기화
    ...
    private void Stop_Record()
    {
        if (m_type != RecordType.Replay)
        {
            ...
            string str = GetUserInfo.id + "_" +Global.NowToString() ;

            //answer.recordData
            GetUserInfo.recName = str;

            Debug.Log(str + " : recName");

            string path = System.Environment.CurrentDirectory.ToString();

            SavWav.Save(path+"/../../Server/recdata/" + str, m_recordAudioClips[0]); // 날짜 _ ID


            //SavWav.Save("C:/Users/yren/Desktop/"+ str, m_recordAudioClips[0]); // 날짜 _ ID
            //Debug.Log(SavWav.Save("C:/Users/yren/Desktop/"+ str, m_recordAudioClips[0]));

            Debug.Log(MainManager.Instance.Cur_ContentsName + " : MainManager.Instance.Cur_ContentsName");
            if (MainManager.Instance.Cur_ContentsName == "SelfPractice")
            {
                SelfPracticeManager.Instance.EndRecord();
            }
            else
            {
                //녹음 끝나고 탐라멈춰있다면 다시 재생 
                if (GetUserInfo.category1 == "자기주장")
                {
                    GameObject selfAssertion;
                    selfAssertion = FindObjectOfType<SelfAssertion>().gameObject;
                    if (selfAssertion.activeSelf == true)
                    {
                        SelfAssertion.Instance.playableDirector.Play();
                    }
                }
            }
